...
class ProcessMonitor(QObject):
    ...
    def _monitor_process(self):
        """Monitor the process stderr in a separate thread to avoid blocking"""
...
def find_window_by_pid(pid, max_attempts=5):
    """Find all windows belonging to a process with retry logic"""
    result = []
    attempt = 0
...
class VideoReceiver(QWidget):
    def __init__(self, parent=None):
        ...
        self.setFixedSize(960, 540)  # Set fixed size to 960x540

        # Create layout
        self.layout = QVBoxLayout(self)
        self.layout.setContentsMargins(0, 0, 0, 0)  # Remove margins for better embedding
        ...
        # Track window find attempts
        self.window_find_attempts = 0
        self.gst_process = None
        self.process_monitor = None
        
        # Start GStreamer with a delay to ensure our widget is created first
    ...
    def handle_gstreamer_error(self, error_message):
        """Handle errors reported by the GStreamer process"""
    ...
    def find_and_embed_window(self):
        ...
        if not hwnds:
            ...
            # Try again after a delay, up to 3 times
    ...
    def cleanup(self):
        """Clean up resources - call this when removing the widget"""
        # Cleanup process monitor
        if self.process_monitor:
            self.process_monitor.stop()
        
        # Cleanup GStreamer process
        if self.gst_process:
...
if __name__ == "__main__":
    ...
    # Create a main window that will host our widget
    main_window = QMainWindow()
    main_window.setWindowTitle("Video Test")
    main_window.resize(980, 560)
    
    # Create and add our video receiver widget
